...
def mockSAPBTPExpert(btp_expert_task):
    ...
    def summary(btp_expert_task, content):
        ...
        Only address the BTP Expert Task. You do not need to other stuffs in the scope.
        Do not leave out any technical details such as API names, technical terms, etc.:
        "{text}"
        SUMMARY:
        """
        map_prompt_template = PromptTemplate(
            template=map_prompt, input_variables=["text", "btp_expert_task"])
    ...
    class ScrapeWebsiteTool(BaseTool):
        ...
        def _run(self, btp_expert_task: str, url: str):
            return scrape_website(btp_expert_task, url)
    ...
    system_message = SystemMessage(
        content="""You are a world class SAP BTP Expert researcher with a deep expertise in the field of crafting solutions in SAP environments,
                ...
                2/ Only address the BTP Expert Task. You do not need to other stuffs in the scope.
